...
x=$(tar -cvzf  $file_cr -P $1)


#tar and export the file system of container
x=$(docker container export --output="$file_fs" $1)
...
x=$(scp $file_cr $remote_loc) #checkpoint scp
x=$(scp $file_fs $remote_loc) #file system scp 
...
ssh -t $2 "cd $loc/restore/; tar -zxvf $loc/restore/$file_cr; cat $file_fs | docker import --change \"$cmd_line\" - $image_name_new; docker create --name $con_name $image_name_new; docker start --checkpoint-dir=$loc/restore --checkpoint $1 $con_name"
